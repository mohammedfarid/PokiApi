...
import com.farid.pokemonapi.domain.model.StateItem
...
fun PokemonDetailsDto.toPokemonDetail(): PokemonDetail {
    return PokemonDetail(
        ...
        state = stats?.map { statsItem ->
            StateItem(
                stateName = statsItem?.stat?.name,
                stateValue = statsItem?.baseStat?.toString()
            )
        }
    )
}
